...
import { contests, mockLeaderboard } from "@/data/contests";
import { motion } from "framer-motion";
import { Trophy, Calendar, Users, Zap, Award, CheckCircle } from "lucide-react";
      ...
      <div className="flex-1 flex flex-col overflow-y-auto">
        ...
        <main className="p-6 max-w-7xl w-full mx-auto space-y-6 flex-grow text-left">
          ...
          <div className="grid grid-cols-1 lg:grid-cols-3 gap-6 items-start">
            ...
            {/* Sidebar scoreboard preview */}
            <div className="glass-panel p-6 rounded-2xl border border-white/10 space-y-4">
              <h3 className="text-sm font-bold text-white uppercase tracking-wider flex items-center gap-2">
                <Award className="w-4 h-4 text-brand-purple-400" /> Global Standings
              </h3>
              <div className="space-y-3">
                {mockLeaderboard.slice(0, 5).map((user) => (
                  <div key={user.rank} className="flex justify-between items-center text-xs pb-2 border-b border-white/5 last:border-0 last:pb-0">
                    <div className="flex items-center gap-2">
                      <span className="font-mono text-gray-500">{user.rank}.</span>
                      <span className="font-semibold text-white">{user.username}</span>
                    </div>
                    <span className="text-brand-cyan-400 font-bold">{user.score} pts</span>
                  </div>
                ))}
              </div>
            </div>
